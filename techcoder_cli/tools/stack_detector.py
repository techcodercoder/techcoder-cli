"""
Auto-detect the tech stack of the current project.
"""
...
_MARKERS = [
    ...
    ('package.json',   'Node.js/JS',     "You are an expert in Node.js, JavaScript, and TypeScript. Check package.json to understand the exact framework and follow its conventions."),
...
def detect_stack(path: str = '.') -> tuple[str, str]:
    ...
    try:
        extensions = {os.path.splitext(f)[1] for f in os.listdir(path)}
    except OSError:
        return '', ''

    if '.dart' in extensions:
        return 'Flutter/Dart', _MARKERS[0][2]
    if {'.ts', '.tsx'} & extensions:
